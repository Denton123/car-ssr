...
## pc端-富文本插件视频上传源码修改-注意事项
由于修改了依赖node_modules里面的summernote富文本的源码---summernote-lite.js
【重点】因此新交接人员，切记先替换一次源码（下面是已经写好的代码），再执行打包发布操作！！

- summernote-lite.js
`其中修改了： VideoDialog.prototype.createVideoNode 方法`
  ...
  $$1.extend($$1.summernote.lang, {
      'en-US': {
          ...
          video: {
              ...
              insert: 'Insert Video',
              url: 'Video URL',
              providers: '(YouTube, Vimeo, Vine, Instagram, DailyMotion, Youku, tencent, iqiyi or bilibili)'
          },
          link: {
              link: 'Link',
              insert: 'Insert Link',
  ...
  */
  ...
  var VideoDialog = /** @class */ (function () {
      ...
      VideoDialog.prototype.createVideoNode = function (url) {
        ...
        //   判断如果是url 是 youku的：
          if (!youkuMatch && url && url.indexOf("youku.com") >= 0) {
            var youkuMatch = []
            youkuMatch[0] = url
            var urls = url.split("?")[0].split("/")
            var keys = urls.filter(v => {if(v.indexOf('==') >= 0) return v;})
            let key = keys[0] ? keys[0].split('.')[0] : keys[0]
            key = key && key.replace("id_", '')
            key = key && key.replace("==", '')
            youkuMatch[1] = key
          }
        //   如果是url 是 bilibli 的：
          if(url && url.indexOf('bilibili.com') >= 0){
              if(url.indexOf('video') >= 0 ){
                  alert('error ! 不支持的哔哩哔哩地址：'+ url + '\n' +'请使用视频分享中的嵌入代码！')
              }else{
                    var bilibiliMatch =  url;
              }
          }
        // 如果是爱奇艺的：
          if(url && url.indexOf('iqiyi.com') >=0 ){
            if(url.indexOf('video') >= 0){
                alert('error ! 不支持的爱奇艺地址：'+ url + '\n' +'请使用视频分享中的通用代码！')
            }else{
                var iqiyiMatch = url ;
            }
          }
          ...
          else if(bilibiliMatch && bilibiliMatch[0].length){
            $video = $$1('<iframe>')
            .attr('frameborder', 0)
            .attr('height', '513')
            .attr('width', '778')
            .attr('src', bilibiliMatch);
          }
          else if(iqiyiMatch && iqiyiMatch[0].length){
            $video = $$1('<iframe>')
            .attr('frameborder', 0)
            .attr('height', '513')
            .attr('width', '778')
