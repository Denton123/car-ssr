...
<template>
  <div>
      ...
      <el-tab-pane label="周排行榜"
        ...
        <ul class="contentSort">
          <li v-for="(item, index) in rankWeekLists"
            :key="index">
            <router-link :to="`/rank/detail/${item.essayId}`">{{item.title}}</router-link>
          </li>
        </ul>
      </el-tab-pane>
    ...
    <router-link :to="`${model}rankList/${activeName == 'first' ? 'w':'m'}/1`"
      class="rankMore"
      v-if="fullPath[0] != '/w' || fullPath[0] !='/m'">更多></router-link>
    <router-link :to="`/index`"
      class="rankMore"
      @click.native="refresh"
      v-if="fullPath[0] == '/w' || fullPath[0] =='/m'">更多></router-link>
    <div class="triangleMask"></div>
  </div>
</template>
...
.todayTabChange .el-tabs__active-bar.is-top {
  ...
  height: 7px;
  background: linear-gradient(-150deg, transparent 5px, #bc081e 0) !important;
  bottom: -17px;
  position: absolute;
  left: -9px;
}
