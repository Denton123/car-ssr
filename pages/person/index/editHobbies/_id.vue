                  ...
                  <el-form :model="hobbyForm"
                    ...
                    <el-form-item>
                      <p class="hobby_form_title">
                        <i>*</i>上传图片<span style="color: #be001e;font-weight: bold">请选择图片710*420px，大小建议为5M以内</span>
                      </p>
                    ...
                    <el-form-item prop="Video">
                      ...
                      <el-upload :action="imgUrl"
                        ...
                        <div slot="tip"
                             class="el-upload__tip">视频大小建议2M,支持格式MP4</div>
                        <i class="el-icon-plus"></i>
                      </el-upload>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取hobbies文章信息
      async getHobbiesInfo() {
        let res = await $get(webHobbiesInfo, { id: this.id })
        this.hobbyForm = res.data
        this.hobbyForm.verifyOptions = []
        // 选中对象默认模块
        this.hobbyForm.verifyOptions.push(this.hobbyForm.classfiy)
        // 文章图片地址拼接显示
        this.hobbyForm.fileList = this.hobbyForm.photoList.map(function(item) {
          return { name: 'img', url: systemManage.getApi(item.photo) }
        })
      },
      // 获取兴趣类别
      async gethobbyType() {
        ...
        this.$refs[formName].validate(valide => {
          if (valide) {
            this.$confirm('此操作将新建兴趣部落文章发布, 是否继续?', '提示', {
              ...
              .then(() => {
                let token = ''
                if (localStorage.getItem('userMsg')  && localStorage.getItem('userMsg') != '' && localStorage.getItem('userMsg').token != '' ) {
...
<style lang="less">
  ...
  .el-breadcrumb__separator:last-child {
    &::before {
      ...
      width: 6px;
      height: 8px;
      background: url('~static/images/small_right_black.png') no-repeat center;
    }
  }
  .publish_hobby {
    height: 100%;
...
.el-dialog__wrapper .imgSize{
  ...
  + .el-radio-button__inner {
    width: 89px;
    height: 42px;
    background: url('~static/images/select_radio_active.png') no-repeat
    transparent;
    background-size: 100% 100%;
  ...
  .hobby_container .hobby_btn.publish {
    color: #f6f6f6;
    background: url('~static/images/person_publish.png');
  }
  .hobby_container .hobby_btn.draft {
    color: #000;
    background: url('~static/images/person_draft.png');
  }
  // 修改elementUI样式
  .el-dialog__body{
    .imgSize{
