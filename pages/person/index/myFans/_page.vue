    ...
    <div class="article_fans_list" v-if="Array.isArray(list) && list.length">
      ...
      :key="index">
        ...
        <div class="content_fans_right">
           <div class="watch_btn_before" v-if="!item.fllowornot" @click="watchBloger(item.id)">关注</div>
           <div class="watch_btn_after" v-if="item.fllowornot" @click="cancelBloger(item.id)">已关注</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async fansList(name) {
      ...
      console.log(data)
       ...
       // this.fansPage = _fanList.data.page.totalPage
       // console.log(_fanList.data.page)
    },
    // 搜索
    onSearch(){
  ...
  mounted() {
    ...
    if (this.cookie == '') {
      this.tokenObj = localStorage.getItem('userMsg') && JSON.parse(localStorage.getItem('userMsg')) != '' ?  JSON.parse(localStorage.getItem('userMsg')) :{}
    }
    if (this.tokenObj == null) {
      this.tokenObj = {}
    }
