        ...
        <div class="detail_content">
          ...
          <div class="detail_content_user" style="background: url('~static/header/nav_upload.png');">
            <nuxt-link :to="`/bloger/${essayData.userId}/1`"
              ...
              <img :src="$ImgUrlRelative(formatPic(userInfo.photo))"
            ...
            <!-- <span class="detail_content_user_desperate"
              v-if="essayData">|</span> -->
            <!-- <span class="detail_content_user_desc"
              v-if="essayData">{{essayData.showTag == null ? '': essayData.showTag}}</span> -->
            <span class="detail_content_time"
              v-if="essayData">发布时间：{{essayData.createTime == null ? '': essayData.createTime}}</span>
          </div>
              ...
              <div class="detail_comment_lists">
                <div v-for="(list, index) in solveCommentList"
                  ...
                  <div class="detail_comment_lists_content_user">
                    ...
                    <p class="detail_comment_lists_content_user_content"
                      v-html="list.commentText"></p>
                  </div>
                  <!-- 回复列表 -->
                ...
                <nuxt-link :to="essayData.userId !== user.id ? `/bloger/${essayData.userId}/1` : '/person/myessay/1'">
                  <div class="detail_user_msg_avatar_wrap">
                    <img v-if="userInfo.photo !== ''"
                      ...
                      :src="$ImgUrlRelative(formatPic(userInfo.photo))">
                    <img v-else
...
import {
  webHobbiesDetailInfo,
  webHobbiesDetailComment,
  webHobbiesDetailReplyComment,
  webHobbiesDetailNewComment,
  webHobbiesDetailChangeLike,
  webEssayClickEssay,
  webHobbiesDetailClickGoodOrBad,
  webUserClickCare,
  webHobbiesDetailTag,
...
// 轮播图
import Slider from 'components/hobbiesdetail/swiper.vue'
...
export default {
  ...
  async asyncData ({params, req }) {
    ...
    // 评论信息
    let articleCommentData = await $get(webHobbiesDetailComment, {
      hobbiesId: params.id,
      limit: '10',
      page: 1
    })
    ...
    let _digestMeta = hobbiesIdDetailData.data.result_data.hobbies.description.slice(0,120)
    let tagMeta= hobbiesIdDetailData.data && hobbiesIdDetailData.data.result_data && hobbiesIdDetailData.data.result_data.hobbies && hobbiesIdDetailData.data.result_data.hobbies.tag
    return {
      ...
      digestMeta: hobbiesIdDetailData.data.result_data && hobbiesIdDetailData.data.result_data.hobbies && hobbiesIdDetailData.data.result_data.hobbies.description.replace(/<\/?.+?>/g,"").replace(/ /g,"").slice(0,120),
      tagMeta
    }
  },
  created(){
  ...
  methods: {
    ...
    // 转义表情
    handleEmotion(i, e) {
      // this.content += i.replace(/\#[\u4E00-\u9FA5]{1,3}\;/gi, this.emotion)
      // alert(1)
      ...
      if(this.tokenObj.token !== undefined || this.cookie !== ''){
       ...
       // 因为这种操作会使得 &nbsp; 的首尾多个<div></div>标签，即不为空了
        ...
        if ( !editor.innerHTML || editor.innerHTML == '' ||
          ...
          let res = await $post(webHobbiesDetailNewComment, urlParam, {
            'X-Auth0-Token':
    ...
    async getArticleData() {
      ...
      if (
        ...
        // console.log(typeof this.badPercent, 'badPercent')
        this.isCanDown = true
        this.isCanUp = true
      } else {
    ...
    async getDataWithTag() {
      let res = await $get(webHobbiesDetailTag, {
        hobbiesId: this.hobbiesid
      })
    ...
    async getCommentData(page = 1) {
      // 获取文章评论信息
      let articleComment = await $get(webHobbiesDetailComment, {
        hobbiesId: this.hobbiesid,
        limit: '10',
        page: page
      })
      this.articleCommentData =
        articleComment.data.list == null ? [] : articleComment.data.list
      this.commentData = articleComment.data
      this.articleCommentSize = articleComment.data.totalCount
    },
    pageChange(page) {
      this.$router.push({
        path: this.$replaceDetailUrl(`/hobbies/hobbiesdetail/${this.hobbiesid}/${page}`)
      })
      this.getCommentData(page)
      this.currentPage = page
      // console.log(this.currentPage, 'curremt')
    ...
    async handleUp() {
      ...
      let urlParam = new URLSearchParams()
      urlParam.append('hobbiesId', this.hobbiesid)
      urlParam.append('good_bad', 2)
      let res = await $post(webHobbiesDetailClickGoodOrBad, urlParam, {
        'X-Auth0-Token': this.cookie !== '' ? this.cookie : this.tokenObj.token
      })
      // console.log(res)
      // console.log(res.data.result_data + 'result_data')
      // 判断是否登录
      if (res.data.result_code === 0) {
        ...
        this.goodPercent = res.data.result_data.goodCount + '%'
        this.badPercent = res.data.result_data.badCount + '%'
        this.getArticleData()
      }
    },
    // 爆胎
...
.detail_content_operate_share_type img {
  width: 37px;
  height: 36px;
  vertical-align: middle;
  margin-right: 19px;
